...
#
# ActiveHumanObservation active inference evaluation (single-person):
#   place person_000 -> before view -> AHO score map
#   -> pixel backprojection -> after view.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="AHO active inference evaluator (single-person)")
...
def main() -> None:
    ...
    scenes_failed = 0
    unique_scenes_done = 0

    all_summaries: list[dict] = []

    for scene_cfg in scene_configs:
        if unique_scenes_done >= max_scenes:
            break
        ...
        try:
            for pos_idx in range(num_positions_scene):
                ...
                pair_center_xy = (float(person_xyz[0]), float(person_xyz[1]))
                ...
                for i, pose in enumerate(ring_samples):
                    certain = _has_full_width_occupancy_visibility(
                        occupancy_map=occupancy_map,
                        person_position_xy=(float(t0["position"][0]), float(t0["position"][1])),
                ...
                lbl0 = targets[0]["semantic_label"]
                for idx_u, i in enumerate(uncertain_indices):
                    pose = ring_samples[i]
                    yaw = _camera_yaw_from_person((pose["x"], pose["y"]), pair_center_xy)
                    visible = int(uncertain_visible_by_target[lbl0][idx_u])
                    total = int(uncertain_total_by_target[lbl0][idx_u])
                    score_field.append(ScoreFieldPoint(
                        x=pose["x"], y=pose["y"], z=pose["z"],
                        camera_z=pose["camera_z"], yaw_rad=float(yaw),
                        score=_score_from_counts(visible, total), distance_m=pose["distance_m"],
                        scoring_mode="segmentation_visibility",
                    ))

                # 每个 pos 固定取 4 个 view，按难度段各取 1 个
                # very hard [0.15,0.30) / hard [0.30,0.45) / medium [0.45,0.60) / easy [0.60,0.75)
                # 某段为空时，从剩余候选中优先补充更难的段，依次往后
                TIERS = [
                    ("very hard", 0.15, 0.30),
                    ("hard",      0.30, 0.45),
                    ("medium",    0.45, 0.60),
                ...
                t0 = targets[0]
                tier_buckets: dict[str, list] = {name: [] for name, _, _ in TIERS}
                for sf in score_field:
                    in_fov = _world_point_in_fov(
                        world_xyz=(float(t0["position"][0]), float(t0["position"][1]), float(t0["position"][2]) + 1.0),
                        camera_xy=(sf.x, sf.y),
                        camera_z=camera_z,
                        yaw_rad=sf.yaw_rad,
                        resolution=resolution,
                    ...
                    for name, lo, hi in TIERS:
                        if lo <= sf.score < hi:
                            tier_buckets[name].append(sf)
                            break

                rng = np.random.default_rng(pos_seed)
                tier_names = [t[0] for t in TIERS]
                for name in tier_names:
                    rng.shuffle(tier_buckets[name])

                # 先每段各取 1 个
                selected = []
                for name in tier_names:
                    if tier_buckets[name]:
                        selected.append(tier_buckets[name].pop(0))
                        print(f"[AHO-EVAL] {pos_tag}: tier={name} score={selected[-1].score:.3f}")
                    else:
                        print(f"[AHO-EVAL] {pos_tag}: tier={name} no candidates")

                # 不足 4 个时，从剩余候选补充（优先更难的段）
                deficit = 4 - len(selected)
                if deficit > 0:
                    for name in tier_names:
                        while tier_buckets[name] and deficit > 0:
                            sf = tier_buckets[name].pop(0)
                            selected.append(sf)
                            print(f"[AHO-EVAL] {pos_tag}: tier={name}(补) score={sf.score:.3f}")
                            deficit -= 1

                if not selected:
                    print(f"[AHO-EVAL] {pos_tag}: no candidates in any tier; skipping pos.")
